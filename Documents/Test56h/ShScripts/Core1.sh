...
cd $loc
echo "MAX2769 Sampfreq:2e6 6.5hr ARM Static: $diff" >> ../output/times.txt
cd /6TB/nfsshare

while [ true ]
	do
	cmp weekend-results/REFtimingaptBinaries_0_0.bin weekend-results/timingaptBinaries_0_0.bin > weekend-results/DetermARM.txt
	cmp weekend-results/REFtimingrnxBinaries_0_0.bin weekend-results/timingrnxBinaries_0_0.bin >> weekend-results/DetermARM.txt
	if ! [ -s weekend-results/DetermARM.txt ]
		then
		break
	else
		mv weekend-results/timingaptBinaries_0_0.bin weekend-results/REVISEDtimingaptBinaries_0_0.bin
		mv weekend-results/timingrnxBinaries_0_0.bin weekend-results/REVISEDtimingrnxBinaries_0_0.bin
		sshpass -p $password ssh -o StrictHostKeyChecking=no root@$arm_ip '/archive/pyxis &> /dev/null'
		cmp weekend-results/REVISEDtimingaptBinaries_0_0.bin weekend-results/timingaptBinaries_0_0.bin > weekend-results/DetermARM2.txt
		cmp weekend-results/REVISEDtimingrnxBinaries_0_0.bin weekend-results/timingrnxBinaries_0_0.bin >> weekend-results/DetermARM2.txt
		break
	fi
done
cd $loc
...
sed -i "/parentpath =/c\parentpath = '\/6TB\/nfsshare\/weekend-results\/';" AnalysisRNXScript1.m
sed -i "/plotpath =/c\plotpath = '\/6TB\/nfsshare\/weekend-results\/Plots\/';" AnalysisRNXScript1.m
sed -i "/truthStr = /c\truthStr = {};" AnalysisRNXScript1.m
sed -i "/ResY = /c\ResY = importWeek('\/6TB\/nfsshare\/weekend-results\/Plots/ResY.txt');" SaveResultsDHT.m
sed -i "/ResW = /c\ResW = importWeek('\/6TB\/nfsshare\/weekend-results\/Plots/ResW.txt');" SaveResultsDHT.m
